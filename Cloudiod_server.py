    ...
    if len(idxs) > 0:
        ...
        for i in idxs.flatten():
            print("detected item:{}, accuracy:{}, X:{}, Y:{}, width:{}, height:{}".format(LABELS[classIDs[i]],
                                                                                             ...
                                                                                             boxes[i][2],
                                                                                             boxes[i][3]))
            #now store these information in a dict
            obj ={
...
## we no longer need to check the command line
# if len(sys.argv) != 3:
...
import json
import base64
# create flask and restful api
app = Flask(__name__)
api = Api(app)

@app.route("/")
def hello():
    return "Hello World!, test page:) good day mate"

#modify the main function now
@app.route('/p2', methods=['POST'])
def main():
    image_data = json.loads(request.get_json())
    image = image_data['image']
    # because the image data is decoded before post
    decoded_data = base64.b64decode(image)
    # because the data is not in byte order, so need to specify the data type
    output = np.frombuffer(decoded_data, dtype=np.uint8)
    ...
    final_obj = {}
    final_obj['id'] = image_data['id']

    final_obj['objects'] = do_prediction(image, nets, Lables)
